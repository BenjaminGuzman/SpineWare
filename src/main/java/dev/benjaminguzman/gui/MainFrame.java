...
import dev.benjaminguzman.gui.notifications.StartUpNotification;
import dev.benjaminguzman.gui.sections.AbstractSection;
import dev.benjaminguzman.gui.sections.BreaksPanel;
import dev.benjaminguzman.gui.sections.CVPanel;
import dev.benjaminguzman.gui.sections.HelpPanel;
...
public class MainFrame extends JFrame
{
	...
	public MainFrame()
	{
		...
		this.setIconImage(SpineWare.getSWIcon());
	...
	private JPanel createMenuPanel()
	{
		...
		try (InputStream inputStreamSWLogo = SpineWare.getFileAsStream("/resources/media/SpineWare_white.png")) {
		...
		for (short i = 0; i < buttonsIconsPaths.length; ++i) {
			button = new JButton(SpineWare.messagesBundle.getString(buttonsLabels[i]));
			button.setIcon(SpineWare.readAndScaleIcon("/resources/media/" + buttonsIconsPaths[i]));
	...
	private void configSysTray()
	{
		...
		try (InputStream iconInputStream = SpineWare.getFileAsStream(iconPath)) {
		...
		sysTrayMenu = new SysTrayMenu(
			this,
			(java.awt.event.ActionEvent evt) -> SpineWare.exit(),
