...
import dev.benjaminguzman.gui.hooksconfig.AbstractHooksConfigDialog;
import dev.benjaminguzman.gui.hooksconfig.ActiveHoursHooksConfigDialog;
import dev.benjaminguzman.gui.hooksconfig.BreakHooksConfigDialog;
import dev.benjaminguzman.hooks.BreakHooks;
import dev.benjaminguzman.prefs.timers.HooksPrefsIO;
import dev.benjaminguzman.prefs.timers.TimersPrefsIO;
import dev.benjaminguzman.timers.breaks.ActiveHours;
import dev.benjaminguzman.timers.breaks.BreakConfig;
import dev.benjaminguzman.timers.breaks.BreakType;
import dev.benjaminguzman.gui.util.NotificationLocationComponent;
...
public class BreaksPanel extends AbstractSection
{
	...
	public void initComponents()
	{
		...
		ImageIcon hooksConfigIcon = SpineWare.readAndScaleIcon("/resources/media/task_white_18dp.png");
		ImageIcon saveConfigIcon = SpineWare.readAndScaleIcon("/resources/media/save_white_18dp.png");
	...
	)
	{
		...
		hooksConfigBtn.setToolTipText(SpineWare.messagesBundle.getString("hooks_config_tooltip"));
	...
	)
	{
		...
		saveConfigBtn.addActionListener((ActionEvent evt) -> {
			...
			if (end_time <= start_time) {
				...
				statusLabel.setText(SpineWare.messagesBundle.getString("active_hours_time_invalid"));
				return;
			}
			...
			try {
				...
				statusLabel.setText(SpineWare.messagesBundle.getString("changes_saved"));
			} catch (InstantiationException e) {
				Loggers.getErrorLogger().log(Level.SEVERE, "Couldn't save preferences", e);
				statusLabel.setForeground(Colors.RED);
				statusLabel.setText(SpineWare.messagesBundle.getString("error_while_saving_changes"));
			}
		});
		saveConfigBtn.addFocusListener(onFocusLostClearLabel);
		...
		panel.add(new NotificationLocationComponent(
			(NotificationLocation selectedLocation) -> {
				if (selectedLocation == cvNotifLocation)
					SpineWare.showErrorAlert(
						SpineWare.messagesBundle.getString(
	...
	private static class OnActionCheckBoxListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			statusLabel.setText(SpineWare.messagesBundle.getString(
				is_enabled
	...
	private static class OnSaveConfigListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (postponeTimeInput.getTime() > workingTimeInput.getTime()) {
				statusLabel.setForeground(Colors.RED);
				statusLabel.setText(
					SpineWare.messagesBundle.getString("working_time_greater_than_postpone_time")
				);
				return;
			} else if (breakTimeInput != null && breakTimeInput.getTime() > workingTimeInput.getTime()) {
				statusLabel.setForeground(Colors.RED);
				statusLabel.setText(
					SpineWare.messagesBundle.getString("working_time_greater_than_break_time")
				);
				return;
			}
			...
			this.statusLabel.setText(
				SpineWare.messagesBundle.getString("changes_saved")
					+ ". " + SpineWare.messagesBundle.getString("changes_saved_extra_text")
			);
	...
	private static class OnSetRecommendedValues implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			// notify the user
			this.statusLabel.setText(SpineWare.messagesBundle.getString("recommended_values_were_set"));
